...
describe 'apache::vhost define', :unless => UNSUPPORTED_PLATFORMS.include?(fact('osfamily')) do
  ...
  context 'new proxypassmatch vhost on port 80' do
    it 'should configure an apache proxypassmatch vhost' do
      pp = <<-EOS
        class { 'apache': }
        apache::vhost { 'proxypassmatch.example.com':
          port    => '80',
          docroot => '/var/www/proxypassmatch',
          proxy_match => [
            { 'path' => '^/(.*\.jpe?g(/.*)?)$', 'url' => 'http://backend.example.com:8001'},
          ],
        }
      EOS
      apply_manifest(pp, :catch_failures => true)
    ...
    describe file("#{$vhost_dir}/25-proxy.example.com.conf") do
      it { should contain '<VirtualHost \*:80>' }
      it { should contain "ServerName proxypassmatch.example.com" }
